...
void  g_free(void *ptr);	    /* free allocated memory */
char *g_strdup(char *s);	    /* save a string in allocated memory */
void *g_new0(size_t n, int count);  /* g_new0: calloc and report if error */
void *g_new(size_t n, int count);   /* g_new: malloc and report if error */
void *g_realloc(void *p, size_t n); /* g_realloc: malloc and report if error */
void  g_assert(int i);		    /* test an assertion */
