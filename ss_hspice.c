...
sf_rdhdr_hspice(char *name, FILE *fp)
{
    ...
    /* first character of binary file is '\000' */ 	
    if((c & 0xff) < ' ') {
...
sf_rdhdr_hsascii(char *name, FILE *fp)
{
    ...
    /* check for supported version of post format */
    if(strncmp(&lbuf[16], "9007", 4) != 0 && strncmp(&lbuf[16], "9601", 4) != 0) {
        strncpy(nbuf, &lbuf[16], 4); nbuf[4] = '\0';
	ss_msg(DBG, "sf_rdhdr_hsascii", "bad hspice file format:\"%s\"\n", name);
	return(NULL);
    }

    strncpy(nbuf, &lbuf[0], 4);	 	
    nbuf[4] = '\0';
    nauto = atoi(nbuf);		/* number of automatically recorded variables in file */
                                /* first one is independent variable (time or freq) */

    strncpy(nbuf, &lbuf[4], 4);
    nbuf[4] = '\0';
    nprobe = atoi(nbuf);	/* number of output variables added by .graph .probe & .plot */
    ...
    /* lines making up a fixed-field structure with variable-types
     * and variable names.  variable names can get split across
     * lines!  so we remove newlines, paste all of the lines
     * together, and then deal with the whole header at once.  A
     * variable name of "$&%#" indicates the end! 
     */

    line = (char *) g_new0(sizeof(char), linesize);
    lineused = 0;
    do {
	    int len;
	    if(fgets(lbuf, sizeof(lbuf), fp) == NULL)
	    ...
	    if(lineused + len + 1 > linesize) {
		    ...
		    line = (char *) g_realloc(line, linesize);
	    }
	    strcat(line, lbuf);
	    lineused += len;
...
hs_process_header(int nauto, int nprobe, int nsweepparam, char *line, char *name)
{
    ...
    sf->ivar->name = (char *) g_strdup(signam);
	...
	sf->dvar[i].name = (char *) g_strdup(signam);
    }

    /* sweep parameter names */
    for(i = 0; i < sf->nsweepparam; i++) {
	if((signam = strtok(NULL, " \t\n")) == NULL) {
	    ss_msg(DBG, "hs_process_header", 
	    	"%s: not enough sweep parameter names found on header line", name);
	    goto fail;
	}
	sf->spar[i].name = (char *) g_strdup(signam);
    }
    return(sf);
...
sf_getval_hsascii(SpiceStream *sf, double *val)
{
    ...
    vbuf[11] = 0;
    if(strlen(vbuf) != 11) {
	/* incomplete float value - probably truncated or partially-written file */
	return(0);
    }
    vp = vbuf;
...
sf_readrow_hsascii(SpiceStream *sf, double *ivar, double *dvars)
{
	...
	if(sf_getval_hsascii(sf, &dvars[i]) == 0) {
	    ss_msg(WARN, "sf_readrow_hsascii", 
		"%s: EOF or error reading data field %d in row %d of table %d; file is incomplete.",
                sf->filename, i, sf->read_rows, sf->read_tables);
	    return(0);
	}
    }
    return(1);
...
sf_readrow_hsbin(SpiceStream *sf, double *ivar, double *dvars)
{
	...
	return(-1);
    } else if(*ivar >= 1.0e29) { /* "infinity" at end of data table */
	sf->read_tables++;
	if(sf->read_tables == sf->ntables) {
