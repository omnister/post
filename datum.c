...
DATUM *twovectors( BINOP op, DATUM *a, DATUM *b) {
	...
	if (c == 0) {
	    ...
	    emit(op, &result, &tmax, &datuma2, zint(datuma2.iv, &datumb2, &datumb));
	    emit(op, &result, &tmax, zint(datumb.iv, &datuma2, &datuma), &datumb);
	    /* emit(op, &result, &tmax, ta2, va2, xinterp(ta2, tb2, vb2, tb, vb)); */
	    /* emit(op, &result, &tmax, tb,  xinterp(tb, ta2, va2, ta, va),  vb); */
	    retb = readnext(&listb, &datumb2, &datumb);

	} else if (c==12 || c == 13 ) {
	    emit(op, &result, &tmax, zint(datumb2.iv, &datuma2, &datuma), &datumb2);
	    emit(op, &result, &tmax, zint(datumb.iv, &datuma2, &datuma), &datumb);
	    /* emit(op, &result, &tmax, tb2, xinterp(tb2, ta2, va2, ta, va), vb2); */
	    /* emit(op, &result, &tmax, tb,  xinterp(tb, ta2, va2, ta, va), vb); */
	    retb = readnext(&listb, &datumb2, &datumb);

	} else if (c==8 || c == 4 || c == 3) {
	    emit(op, &result, &tmax, &datuma2, zint(datuma2.iv, &datumb2, &datumb));
	    emit(op, &result, &tmax, &datuma, zint(datuma.iv, &datumb2, &datumb));
	    /* emit(op, &result, &tmax, ta2, va2, xinterp(ta2, tb2, vb2, tb, vb)); */
	    /* emit(op, &result, &tmax, ta,  va,  xinterp(ta, tb2, vb2, tb, vb)); */
	    reta = readnext(&lista, &datuma2, &datuma);
	...
	switch(op) {
		...
		*result = link_dat(*result, tmp);
		...
		if ((datuma_old.re > datumb_old.re && datuma->re < datumb->re) ||
		   ...
		   tmp = scalar(op, 
		   	zint(tint, &datuma_old, datuma),
		   	zint(tint, &datumb_old, datumb) 
		   );
		   tmp->iv = tint;
		   *result = link_dat(*result, tmp); 
	        }
