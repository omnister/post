...
void freespicedat(SPICEDAT *sp) {
    int i;
    if (sp->title != NULL) free(sp->title);
    if (sp->date != NULL) free(sp->date);
    if (sp->plottype != NULL) free(sp->plottype);
    if (sp->filename != NULL) free(sp->filename);
...
SPICEDAT *sp_read(char *filename)
{
	...
	while ((sp = sptab[ntab] = read_header(fp)) != NULL) {
	    ...
	    plotnum = ntab;	// make this the current rawfile

	    printf("	loaded analysis %d\n", ntab);
...
void se_list(void) {
	...
	if (i==cur_simno()) {
       ...
       printf("# %s - %s %s\n", chomp(sp->filename), chomp(sp->plottype), chomp(sp->date));
       printf("\t    # title: %s", sp->title);
    }
    if (ntab==0) {
       printf("\tno raw file loaded: use ci \"file.raw\" to load simulation\n");
    }
}

char *chomp(char *s)
{                               // get rid of newlines and <cr>
    int n;
    char *initial = s;
    char *start = s;
    int debug = 0;
    ...
    n = strlen(s);
    if (debug) {
        printf("chomp called with n %d: <%s>\n", n, s);
    }
    s += (n - 1);
    while (*s == '\n' || *s == '\r' || *s == 0x0d) {
        *s = '\0';
